  ...
  for (bindex=0,i=0; i<nsets; i++)
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  /* invalidate new cache block */
        ...
        //Implement by group************************************************************************start
        //Initialisa priority.
        blk->priority = 0;
        //Implement by group**************************************************************************end
...
//Implement by group************************************************************************start
//update priority
void update_priority(struct cache_blk_t * blk , int operation){
    //increase priority
    if(operation){
        if( blk->priority ==3 ){
            //equal to maxmum priority, then do nothing.
            return;
        }else{
            blk->priority += 1;
        }
        ...
        if( blk->priority == 0){
            //equl to minimum priority, then do nothing.
            return;
        }else{
            blk->priority -= 1;
        }
...
//Implement by group************************************************************************start
//priority LRU policy implementation, select victim.
struct cache_blk_t * select_victim(struct cache_blk_t * blk1, struct cache_blk_t * blk2, struct cache_blk_t * blk3, struct cache_blk_t * blk4){
    /*select way with smallest priority as victim
     when there are more than one way, then select way
     which closest to LRU position*/
    struct cache_blk_t * result = blk1;
    if( blk2 && blk3 && blk4){
        int mini = blk1->priority;
        struct cache_blk_t * tmp_list[4] = {blk1, blk2, blk3, blk4};
        for(int i = 0; i < 4; i++){
            if( tmp_list[i]->priority < mini ){
                mini = tmp_list[i]->priority;
                result = tmp_list[i];
            }
            update_priority(tmp_list[i], 0);
        }
    }
    result->priority = 1;
    return result;
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if (CACHE_TAGSET(cp, addr) == cp->last_tagset)
    {
      ...
      blk = cp->last_blk;
        //Implement by group************************************************************************start
        //Update priority of current way.
        update_priority(blk, 1);
        //Implement by group**************************************************************************end
      goto cache_fast_hit;
    }
  ...
  cp->misses++;
    //Implement by group************************************************************************start
    //define 4 temporory ways which used to find victim.
    struct cache_blk_t* blk1 = NULL;
    struct cache_blk_t* blk2 = NULL;
    struct cache_blk_t* blk3 = NULL;
    struct cache_blk_t* blk4 = NULL;
    //Implement by group**************************************************************************end
  ...
  case PriLRU:
          if(cp->assoc<=4){
              ...
              blk1 = cp->sets[set].way_tail;
              blk2 = blk1->way_prev;
              blk3 = blk2->way_prev;
              blk4 = blk3->way_prev;
              repl = select_victim(blk1, blk2, blk3, blk4);
          }
 ...
 cache_hit: /* slow hit handler */
  ...
  /* if LRU replacement and this is not the first element of list, reorder */
    //Implement by group**************************************************************************start
  if (blk->way_prev && cp->policy == LRU && cp->policy == PriLRU)       //'&& cp->policy == PriLRU'
      //Implement by group**************************************************************************end
    {
